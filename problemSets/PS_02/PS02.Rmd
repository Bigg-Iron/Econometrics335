...
packages <- c("tidyverse", "dplyr", "moments", "lmtest", "sandwich", "stringr", "readr", "here", "ggplot2", "modelr", "MASS")
...
confint(model_a, level = 0.99) # 1% level: Reject the hypothesis that the coefficient on the unemployment rate is zero at the 1% level.
confint(model_a, level = 0.95) # 5% level: Reject the hypothesis that the coefficient on the unemployment rate is zero at the 5% level.
confint(model_a, level = 0.90) # 10% level: Reject the hypothesis that the coefficient on the unemployment rate is zero at the 10% level.
...
```{r (b)}
summary.lm(modelA <- lm(formula = rural_atlas_merged$UnempRate2013 ~ rural_atlas_merged$PerCapitaInc))

coeftest(modelA, vcov = vcovHC(modelA, type = "HC0"))
